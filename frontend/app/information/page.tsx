  ...
  useEffect(() => {
    const fetchInfo = async () => {
      try {
        ...
        const endpoint = `${baseUrl}/api/api/info/`;
        ...
        // Filter only published items if needed
        const publishedItems = data.filter(item => item.is_published !== false);
      ...
      <section className="relative bg-white py-24 min-h-screen overflow-hidden">
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
            <p className="text-red-600">Error loading information: {error}</p>
            <p className="text-sm text-gray-500 mt-2">
              Attempted to fetch from: {process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:8000'}/api/api/info/
            </p>
            <button 
          ...
          {infoItems.map((item) => (
            ...
            >
              ...
              <div className="flex flex-col flex-grow p-5">
                ...
                {/* --- READ MORE LINK --- */}
                <div className="mt-3 inline-flex items-center gap-1 text-sm font-semibold text-[#8B1A1A] hover:text-[#a02020] transition-colors">
                  थप पढ्नुहोस्
                  <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" /></svg>
                </div>
              </div>
